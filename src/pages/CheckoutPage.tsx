...
export function CheckoutPage() {
  ...
  if (cart.length === 0) {
    return (
      <div className="container-max px-4 py-20 text-center">
        <h1 className="text-2xl font-bold text-white mb-4 font-display">Your cart is empty</h1>
  ...
  return (
    <div className="container-max px-4 sm:px-6 lg:px-8 py-8">
      <button onClick={() => navigate({ name: 'cart' })} className="flex items-center gap-1.5 text-sm text-navy-400 hover:text-white mb-6 transition-colors">
        <ArrowLeft className="w-4 h-4" />
        Back to cart
      </button>

      <h1 className="text-3xl font-extrabold text-white mb-8 font-display">Checkout</h1>
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              {[
                ...
                <button
                  ...
                  className={`flex items-center gap-3 p-4 rounded-2xl ring-1 transition-all ${
                    deliveryOption === opt.id ? 'ring-emerald-500 bg-emerald-500/10' : 'ring-navy-700 hover:ring-navy-600'
                  }`}
                >
                  <div className={`w-5 h-5 rounded-md flex items-center justify-center ${deliveryOption === opt.id ? 'bg-emerald-500' : 'ring-1 ring-navy-700'}`}>
                    {deliveryOption === opt.id && <Check className="w-3.5 h-3.5 text-white" />}
                  </div>
                  <opt.icon className="w-5 h-5 text-navy-200" />
          ...
          <div className="card p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-sm font-semibold text-navy-200 mb-1.5 block">Phone</label>
                <div className="relative">
                  <Phone className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-navy-500" />
                  <input
                    type="tel"
              ...
              <div className="sm:col-span-2">
                <label className="text-sm font-semibold text-navy-200 mb-1.5 block">Email</label>
                <div className="relative">
          ...
          {deliveryOption === 'delivery' && (
            <div className="card p-6 animate-fade-in">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-semibold text-navy-200 mb-1.5 block">Street address</label>
                  <input
                    type="text"
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-semibold text-navy-200 mb-1.5 block">City</label>
                    <input
                      type="text"
                      value={formData.city}
                  ...
                  <div>
                    <label className="text-sm font-semibold text-navy-200 mb-1.5 block">Pincode</label>
                    <input
                      type="text"
          ...
          {/* Notes */}
          <div className="card p-6">
            <h2 className="text-lg font-bold text-white mb-4 font-display">Notes to lender (optional)</h2>
            <textarea
          ...
          <div className="card p-6 lg:sticky lg:top-20">
            ...
            <div className="space-y-3 mb-4 max-h-48 overflow-y-auto">
              {cart.map((item) => (
                <div key={item.product.id} className="flex gap-3 items-center">
                  ...
                  <span className="text-sm font-bold text-white shrink-0">₹{(item.product.pricePerDay * item.days).toLocaleString('en-IN')}</span>
                </div>
              ))}
            </div>

            <div className="border-t border-navy-700 pt-4 space-y-2 mb-4">
            ...
            <div className="border-t border-navy-700 pt-4 mb-6">
              <div className="flex justify-between items-baseline">
                ...
                <span className="text-2xl font-extrabold text-white">₹{(cartTotal + cartDepositTotal).toLocaleString('en-IN')}</span>
              </div>
            </div>

            <div className="rounded-2xl bg-emerald-500/10 p-3 mb-6 flex items-start gap-2">
              <Shield className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
              <p className="text-xs text-navy-200">Deposits refunded automatically upon safe return.</p>
            </div>
